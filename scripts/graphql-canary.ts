...
async function runTemplate(token: string, entry: {readonly name: string; readonly query: string}): Promise<'ok' | 'scope-limited'> {
  ...
  if (errors && errors.length > 0) {
    if (classifyTemplateOutcome(errors) === 'scope-limited') {
...
// rm-232 (was rm-228 pre-renumber): hermetic seam for the canary's verdict
// logic — tests import this instead of the old import-and-exit trap.
export function classifyTemplateOutcome(errors: readonly unknown[]): 'ok' | 'scope-limited' | 'failed' {
  if (errors.length === 0) return 'ok'
  return errors.every(error => isVulnerabilityAlertsPermissionError(new Error(errorMessage(error)))) ? 'scope-limited' : 'failed'
}

export {REPO_STATUS_QUERY_REGISTRY as REGISTRY}
...
// Import-safe entry guard (same pattern as scripts/should-release.ts): main()
// runs only when this file is the invoked program, so tests can import the
// classifier and registry without triggering the token check or CI calls.
if (realpathSync(process.argv[1] ?? '') === realpathSync(new URL(import.meta.url).pathname)) {
  await main()
}
