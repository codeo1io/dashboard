...
describe('listener store', () => {
  ...
  it('rm-227: duplicate ingest is idempotent — no throw, one row, ack state preserved', () => {
    // The old check-then-insert pair raced under concurrent duplicate ingest:
    // both SELECTs missed, both INSERTed, the loser 500'd on the partial
    // UNIQUE index. insert() is now a single INSERT…ON CONFLICT upsert, so a
    // replay can never surface a constraint violation to the webhook caller.
