...
// ---------------------------------------------------------------------------
// rm-112 warm-empty protection composed with resolver failures
// (independent-review finding F2, 9351c56b)
// ---------------------------------------------------------------------------

describe('aggregator — rm-112: warm-empty protection with resolver failures present', () => {
  it('an empty fresh union with resolver failures preserves previously-served install rows stale instead of dropping them', async () => {
    const installRepo = makeRepo({node_id: 'NODE_INSTALL', owner: 'org', name: 'install-row'})
    let cycle = 0
    const deps = makeDeps({
      readMetadata: vi.fn().mockImplementation(async () => {
        cycle++
      ...
      enumerate: vi.fn().mockImplementation(async () => makeEnumerateResult(cycle === 1 ? [installRepo] : [])),
      resolveInstallationIdForRepo: vi.fn().mockRejectedValue(new Error('installation lookup failed')),
      graphqlQueryForInstallation: vi.fn().mockResolvedValue(makeGraphqlResponse({rollupState: 'SUCCESS'})),
    })
    const agg = createAggregator(fakeInstallationsClient, fakeMetadataReader, deps)
    ...
    // Cycle 1: only the install-channel repo exists; fresh and green.
    let snap = agg.getSnapshot()
    expect(snap.repos.map(repo => repo.full_name)).toEqual(['org/install-row'])
    expect(snap.staleBanner).toBe(false)

    // Cycle 2: enumeration genuinely returns zero AND the metadata repo's
    // resolver fails. The fresh resolvable union is empty — the absence row
    // is a failure marker, not fresh data, so the previously-served install
    // row must survive marked stale under the banner instead of being wiped
    // by an absence-rows-only commit.
    await agg.refresh()
    snap = agg.getSnapshot()
    expect(snap.repos.map(repo => repo.full_name).sort()).toEqual(['org/install-row', 'org/meta-row'])
    const install = snap.repos.find(repo => repo.full_name === 'org/install-row')
    expect(install?.status.stale).toBe(true)
    expect(install?.status.rollupState).toBe('green')
    const meta = snap.repos.find(repo => repo.full_name === 'org/meta-row')
    expect(meta?.status.stale).toBe(true)
    expect(meta?.status.rollupState).toBe('unknown')
    expect(snap.staleBanner).toBe(true)
    expect(snap.degraded).toBe(true)
