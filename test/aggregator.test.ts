...
// ---------------------------------------------------------------------------
// rm-151 / rm-154 — secondary-guard precision
// ---------------------------------------------------------------------------

describe('rm-151/rm-154 — denylist secondary guard precision', () => {
  ...
  it('an R_ node_id entry WITH an explicit database_id does NOT fire the partial-protection warning (rm-154)', async () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {})
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
    try {
      const privateRepo = makeRepo({
  ...
  it('an entry with NO database_id in either form still fires the partial-protection warning (rm-154)', async () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {})
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
    try {
