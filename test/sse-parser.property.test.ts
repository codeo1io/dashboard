...
describe('operator SSE parser properties (rm-144)', () => {
  ...
  it('unknown-field transparency: retry:/id:/arbitrary field lines never alter frames', () => {
    fc.assert(
      fc.property(
        fc.array(frameArb, {minLength: 1, maxLength: 10}),
        fc.array(
          fc
            .tuple(fc.constantFrom('retry', 'id', 'custom-field', 'x'), fc.stringMatching(/^[\w .:-]{0,24}$/))
            .map(([k, v]) => `${k}: ${v}`),
          {minLength: 1, maxLength: 4},
        ),
        (frames, fields) => {
          // SSE reserves `retry:`/`id:` for reconnection state this parser
          // deliberately does not keep; any other unknown field is skipped
          // the same way. Interleaving them inside records must not alter
          // the parsed frames (rm-144 review: retry-field coverage).
          const injected = frames
            .map(f => `${fields.join('\n')}\n${serialize(f, '\n')}`)
            .join('')
          const expected = frames.map(f => f.frame)
          expect(parseSseChunk(injected).map(r => (r.success ? r.frame : r.error))).toEqual(expected)
        },
      ),
      {numRuns: 200},
    )
