...
// ---------------------------------------------------------------------------
// rm-180: cached token honors the API-provided expiresAt (no 55-min guess)
// ---------------------------------------------------------------------------

describe('rm-180 — cached token honors the API-provided expiresAt', () => {
  ...
  it('far-future expiresAt (8 h) is capped at the 55-min historical ceiling', async () => {
    ...
    try {
      ...
      // API expiry — the cap preserves the pre-rm-180 refresh cadence.
