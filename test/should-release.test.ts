...
describe('should-release — workflow path filter parity', () => {
  ...
  // guard never executes (the rm-144 drift mode: a missed release). The
  ...
  it('every hard-release corpus entry appears in the workflow paths filter', () => {
    ...
    expect(
      missing,
      'hard-release paths missing from release.yaml on.push.paths — a change to them would never trigger Release (rm-144)',
    ).toEqual([])
  })
  ...
  it('every workflow paths entry is accounted for by the guard corpus or the conscious trigger-only allowlist', () => {
    ...
    expect(
      unaccounted,
      'release.yaml on.push.paths entries unknown to the guard — pair them in scripts/release-paths.ts (corpus or WORKFLOW_TRIGGER_ONLY_PATHS) or remove them (rm-144)',
    ).toEqual([])
  })
})

describe('should-release — pnpm-workspace.yaml change triggers release', () => {
  // rm-144: workspace config carries the security overrides + allowBuilds and
