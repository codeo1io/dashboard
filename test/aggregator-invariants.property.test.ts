...
describe('aggregator redaction invariants (rm-144)', () => {
  ...
  it('redaction leaves unrelated text verbatim', () => {
    fc.assert(
      fc.property(
        // Chain the innocent string off the drawn identity so the filter can
        // see it: the innocent alphabet overlaps the identity alphabets, so a
        // generated innocent CAN contain owner/name as a substring — that
        // collision legitimately redacts and would false-fail "verbatim"
        // (review F1: 6/12000 collisions in fc.sample). Same pattern the SSE
        // suite uses for unknown event names.
        identityArb.chain(entry =>
          fc
            .stringMatching(/^[a-z0-9 ._/-]{1,60}$/)
            .filter(s => !s.includes('REDACTED'))
            .filter(s => !s.includes(entry.owner) && !s.includes(entry.name))
            .map(innocent => ({entry, innocent})),
        ),
        ({entry, innocent}) => {
