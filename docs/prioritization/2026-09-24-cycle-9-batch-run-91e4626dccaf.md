> **LANDING HEADER (2026-09-26, integrate conflict case 0b6d23ce).** This batch landed as candidate
> `8a939a1` (run 91e4626dccaf, PR #45) merged into main's `81bf3ee` lineage via the conductor
> integration `58eb3481d229`. Main's own cycle-9 compound (run 04c7d9c2) had already landed the
> canonical `2026-09-24-cycle-9-batch.md` name — this doc is renamed
> `-run-91e4626dccaf.md` per the path-collision convention; its body is preserved verbatim below
> except the id reconciliation noted here. **ID reconciliation at landing** (landed meanings own
> ids): rm-155, rm-156 and rm-158 landed under their minted ids — main's 2026-09-24 compound had
> audited this branch's ids via `git log --all` and deliberately kept 155/156/158 free for it
> (renumbering its own absorb item to rm-157). This batch's rm-157 (gateway run-status checkout
> provenance, candidate) collided with that landed rm-157 and renumbers to **rm-247**, above the
> all-lineage max rm-246; its three in-body references are updated below. Dispositions are
> recorded atomically in ROADMAP.md's INTEGRATE-MERGE comment for this merge.
...
| rm-247 gateway run-status provenance (34.0) | 6 | 3 | 2 | S–M | agent pin ≥ v0.115.0 = open upstream PR #521 | DEFER — external pin gate |
...
when it merges, rm-247 unblocks and rm-103's absorb window opens.
...
## Outcome — recorded 2026-09-24 (compound phase, pre-review; run 91e4626d attempt 74e984b2)

Status: batch implemented and validated; review/shipping outcomes land after this record and the
next cycle's assessment carries them forward.

- B1 rm-156 (U1) IMPLEMENTED — server GitHub-data-plane timeouts + refresh watchdog.
  Implementation superseded the batch's assumed primitive: the Octokit `timeout` option is inert
  on hung upstreams and `@octokit/graphql` forwards no signal — the shipped bound is
  `request: {fetch: createBoundedFetch(15s)}` (AbortSignal.any with the caller signal) on the
  constructor, the auth-app mint, and graphql.defaults. plugin-retry amplified timeout aborts
  (3 retries → ~14.7s extra stall) and was disabled for this client. Watchdog:
  refreshDurationMs/refreshDegraded (90s ceiling) on every snapshot path, surfaced via
  /api/monitoring. Lesson doc: docs/solutions/runtime-errors/octokit-timeout-option-inert-on-hung-upstreams-bind-at-fetch-layer-2026-09-24.md.
- B2 rm-155 (U2) IMPLEMENTED — Listener latch timeout-race + unmount abort; App unread-poll
  in-flight guard, hidden-tab pause/resume, once-per-outage error surface.
- B3 rm-158 (U3) IMPLEMENTED — W3C badge mirrors unreadCount, cleared after 2 consecutive
  poll failures so it cannot lie during an outage.
- Validation (pre-review): targeted 696/696 (engine impacted resolver, exit 0); full gate =
  engine ephemeral cloud-CI PR #38, ok:true, all checks pass, refs auto-deleted; local
  check-types/lint/build:web green; new hung-server transport tests 4/4.

Next-cycle candidates left by this batch:

1. rm-141 bounded refresh pool — now unblocked; rm-156's watchdog fields are its natural input.
   Only a pool can CAP walk time below repos × 15s; the watchdog only marks degradation.
2. rm-247 operator provenance — still deferred, waits on the agent pin passing v0.115.0
   (upstream #521).
3. Landing-gate hazard (this batch): the ephemeral full-CI head is built from `git diff HEAD`,
   so UNTRACKED files (the new test/app-client-timeout.test.ts) never reached cloud CI —
   the commit gate must stage the explicit 17-file list or those 4 tests ship unvalidated.
4. Housekeeping candidate: ~20 stale `conductor/ci-*` / `conductor/ci-base-*` validation branches
   accumulate on origin from prior re-fire storms (mine were auto-deleted); worth a one-shot
   prune in a maintain pass.
