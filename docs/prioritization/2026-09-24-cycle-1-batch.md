...
> **LANDING HEADER (2026-09-26, integrate of run 6cbc2a1d into main 0ca643e — integration
> 5e10362e6b47, conflict case chain 050989965a → … → f3c5735b → 0b798e55).** This batch sat
> superseded-in-flight ~2 days while main landed cycles 9–17 plus ten sibling integrates; the full
> disposition record lives in ROADMAP.md's INTEGRATE-MERGE comment for candidate b0fc297. Frame-id →
> landed-id map for everything below: rm-155 → folded into rm-119 (fleet status surface);
> rm-156 → **rm-216**; rm-157 → folded into rm-195; rm-158 → folded into rm-218 (rulesets panel);
> rm-159 → folded into rm-117 (main's landed rm-159 = origin validation-residue sweep);
> rm-160 → folded into rm-105; rm-161 (B2 security.txt) → **rm-245**; rm-162 → not carried
> (dormant by design at fleet = 1, recorded in the not-selected list below); rm-163 → residue
> obligations at **rm-217** (coverage half moot — cycle-9's riders widened the CI filters).
> B4 ack-CSRF completes main's rm-115. B1 (pnpm 11.27.1 lockstep) and B3 (toml override,
> GHSA-82x6-q7mm-w9cf) were superseded by main's independent landings (dcef12b; cycle-13 compound
> 753a69d9). The solution doc named in the B3 row was NOT TAKEN at the integrate — it is a
> near-duplicate of main's landed
> `docs/solutions/build-errors/pnpm11-overrides-live-in-pnpm-workspace-yaml-2026-09-24.md`
> (399447b), which is the canonical reference for that trap. Ids below keep their batch-frame
> numbering for historical accuracy; cite the landed ids above.
...
- Operator action folded in at review-F2 (2026-09-24): enable private vulnerability reporting on
  codeo1io/dashboard before or at landing (`gh api -X PUT
  repos/codeo1io/dashboard/private-vulnerability-reporting`; verify with a GET → `"enabled":true`)
  so the shipped security.txt advisory Contact goes live. Until flipped, security.txt's second
  Contact (upstream fro-bot/dashboard issues, probed working 2026-09-24) is the live channel —
  not a ship blocker.
...
## Cycle outcome (2026-09-24, compound phase — pre-review evidence only)

Implementation and validation are recorded here from the implement (attempt a292009c), targeted_tests
(c6cce329), and full_tests (4813d5d6) phases; nothing was re-run in the compound phase.

| Item | Status | Outcome evidence |
|------|--------|------------------|
...
ok:true via ephemeral PR #31 (snapshot 51e97d2, visual SUCCESS — run 35933756022), supplemented by
the complete local suite: **3127/3127** (31 server files / 2047 tests; 28 web files / 1080 tests,
pretest client rebuild included), exactly base 3115 + 5 CSRF + 5 security.txt + 2 net-new web
tests. Gates at implement: `pnpm check-types` 0, `pnpm lint` 0.

**Coverage caveat (compounded to rm-163):** the engine's ephemeral validation PRs target
`conductor/ci-base-*`, but main.yaml's `pull_request` trigger is branch-filtered to `[main]`, so
only the visual workflow executes on them — `ok:true` is visual-only unless the phase runs the
local suite, as above.

### B0 landing checklist (for the commit phase)

1. Reconcile PR #23 (0334a4eb, deletes the tracked `.conductor` breadcrumb) BEFORE any push —
   merging the batch onto a still-red main re-creates the incident.
2. Stage the explicit file list only (15 files: the B1-B4 payload + ROADMAP.md + this doc + the
   new solution doc + contract doc); never `git add -A`/`git add .` — `.conductor/` and the
   canonical checkout's foreign untracked file (docs/solutions/workflow-issues/zz-conflict-lint-check.md)
   must not ride along.
3. After any landing merge: `git ls-files .conductor` must print NOTHING (empty); `git rm --cached`
   any hit (memory #3256 — fresh merges re-introduce the breadcrumb on the OURS side).
4. Observe Main green at the EXACT pushed sha (not the stale tip — #3263), after cancelling any
   stale queued Main runs (#3391).

### Standing obligations created this cycle

- **security.txt Expires 2026-12-24** — refresh quarterly on the maintenance cadence (rm-161;
   pinned at src/server.ts:92). Next due at or before the 2026-12 cycle.
- **CSRF token 1h window** — the listener ack token rotates hourly (current + previous window
   accepted); operators on long-idle sessions re-fetch transparently via GET /api/listener/csrf.

### Independent-review fixes applied (2026-09-24, attempt 3300fe4c)

Review verdict was SHIP-BLOCKED (conditional) on F1; all six findings resolved:

- **F1 (HIGH) fixed** — src/server.ts ackCsrf now derives from the RESOLVED operatorLogin
  (opts with DASHBOARD_OPERATOR_LOGIN fallback) + sessionManager presence, so env-configured and
  createDashboardServer deployments get ack CSRF (was: raw opts → null → 403 'csrf unavailable'
  on every ack in production). Regression test added (env-mode round-trip).
- **F2 (MEDIUM) fixed** — security.txt second Contact repointed to upstream
  fro-bot/dashboard/issues (has_issues=true, probed live 2026-09-24; the fork's own issues are
  dead by fork default and PVR is off). PVR enablement folded into the B0 checklist above as the
  operator action; test pins the contact policy.
- **F3/F4/F5 (LOW) fixed** — ROADMAP version-staleness corrections (11.27.1 lockstep noted in
  rm-108's signal, rm-131's acceptance made version-neutral, rm-115's web-test count corrected to
  5 ack-flow = 3 updated + 2 net-new fail-closed).
- **F6 (LOW) fixed** — rm-131 reconciled to completed (guard landed on origin/main; its firing
  reddened runs 35840489685/35877660739/35878320796/35883003173) and moved to the Completed
  section.
- Re-validation: focused suites re-run at targeted scope (listener-routes 21, server 8, lint,
  check-types) — see the review-fix PhaseResult evidence.

### Context for the next cycle

- Rebase/land strategy: origin/main was last observed red (763c1fa) with the fix in PR #23 — next
  cycle's assessment re-verifies and carries review/landing outcomes forward.
- Deferred with cause: rm-155 (self-inclusion — needs repos.yaml data-branch + App install, outside
  the worktree), rm-146 (auto-merge — blocked behind rm-116 main protection).
- New candidate from validation: rm-163 (ephemeral CI coverage gap).
- Upstream watch: #521 (fro-bot/agent v0.115.0), #519 (bfra-me action) — no pin changes needed.
