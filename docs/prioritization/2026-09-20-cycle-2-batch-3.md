...
restored: '2026-09-24 — this stranded branch landed as -3 per the parallel-campaign collision protocol (docs/solutions/workflow-issues/parallel-conductor-campaigns-frame-movement-collisions-2026-09-20.md): the unnumbered path carries run 270220e7 landed batch, -2 carries 779e7271 re-scope. All ids below are pre-renumbering lineage — mapping at ROADMAP.md manual-revision header: rm-112→rm-116, rm-113→rm-117, rm-114→rm-118, rm-115→rm-119, rm-116→rm-120, rm-117→rm-121 (implemented), rm-118 folded into rm-112; rm-100a/b, rm-110, rm-111 content landed on main in evolved form (fro-bot.yaml gates via job-env + per-step if; CodeQL additionally via SEMMLE_TYPESCRIPT_HOME)'
phase: prioritize
