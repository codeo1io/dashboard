...
and PR #23's title burned "cycle-8" without landing a doc. Taking lineage **cycle 9** avoided both collisions
at authoring time. (Post-hoc note, 2026-09-24 review finding: PR #45 from run 91e4626dccaf later claimed the
cycle-9 label too, so lineage numbers now collide on the label itself; the run id `04c7d9c2…` is the
disambiguator, as the ROADMAP compound header records.)
...
- Worktree ROADMAP hazard: this run's roadmap phase (attempt 4b1665c5) added rm-148..rm-153 at base 7809df6;
  ...
  (release-trigger item → rm-154; rm-149 fro-bot-absorb keeps its id, free at main; final id `rm-157`
  after the 943d3ed audit — see outcome §B0).
...
- **B0 — ROADMAP landing reconciliation (rider, XS).** Merge the worktree ROADMAP (base 7809df6 + this run's
  ...
  rm-148), keep the fro-bot-absorb item as rm-149 (free at main; ultimately renumbered rm-155 at implement
  reconciliation and rm-157 at compound — the ROADMAP header mapping is authoritative), mark rm-150/151/152/153 absorbed with
...
- **B2 — rm-112 fix-half: make degradation visible and the cache real.** Scoped to the three measured gaps:
  ...
  (aggregator.ts:717-718 vs :789-795 at the base), so steady-state reads hit cache only by timing
  luck (interval-equal TTL under a strict `<`); either raise TTL
...
(a) Cache decision: DELETE, not raise. The cache's steady-state hit rate was not meaningfully above
zero (TTL == interval with strict `<`; an entry could only dip under TTL by inheriting wall-time
variance from predecessors' fetches), so removal changes no meaningful behavior while deleting the
dead weight; a
...
### Validation rider (2026-09-24, targeted_tests attempt 409ba5bb)
...
### Cycle-9 compound (2026-09-24, post-validation, attempt 3799ed24)

Pre-review validation outcomes, consumed as recorded evidence:

- **Local authoritative:** full suite 3124/3124 (2046 server + 1078 web, 0
  failed), `pnpm lint` 0, `pnpm check-types` 0, actionlint 0 on edited
  workflows.
- **Cloud ephemeral-PR route** (after the ci-base filter rider): 9 checks
  registered — 8 green, 1 red. The red is the rm-131 fork-exclusion guard
  failing ONLY in the validation clone: main 763c1fa still tracks the
  `.conductor` residue and the engine's `:(exclude).conductor/**` patch
  pathspec drops this batch's staged deletion of exactly that file.
  Self-cures at Unit C's landing; no worktree-side fix exists.

Learnings compounded to durable artifacts:

- ROADMAP.md: cycle-9 compound header block; rm-155 renumbered **rm-157**
  (conductor/run-91e4626dccaf 943d3ed already used rm-155/156/158 — the
  git-log id-audit rule applied with one phase's lag); dated signal appends
  to rm-154 (validated) and rm-131 (the cloud-artifact mechanism); new item
  **rm-159** — origin validation-residue sweep (stale `conductor/ci-*` PRs +
  refs need push authority this run lacks).
- docs/solutions/workflow-issues/ephemeral-ci-base-pr-excluded-by-branch-filters-2026-09-24.md
  — zero-checks dead-wait: diagnosis fast path, the branch-filter fix, the
  SIGINT + nohup-relaunch protocol.
- docs/solutions/workflow-issues/validation-clone-exclude-pathspec-drops-staged-breadcrumb-deletion-2026-09-24.md
  — the cloud-only rm-131 red: mechanism, cure (land the untrack), triage rule.

Next-cycle queue: rm-157 (fro-bot/agent v0.115.0 absorb, needs live gateway),
rm-159 (residue sweep), rm-112 remaining halves (resolver-failure absence
entries, warm-empty banner, MonitoringDto degradation signal), rm-116/rm-144
carried from cycle 7. Watch: dependabot first PRs ~2026-10-03; rm-133
major-pin re-evaluation 2026-10-21.
