...
> **Landing header (2026-09-26, integrate of run 998b4953 into main 83608e9 lineage, conflict case
> 9f604f39 — completed at the 1b1732ae re-fire, seam-fixed and re-staged at the 68adf406 re-fire,
> re-merged content-unchanged onto main 83608e9 at the a5eb0474 re-fire).**
> Authored by conductor run 998b49532b44 (candidate bb659ec, base 7809df6) under the filename
> 2026-09-23-cycle-7-batch.md — a name main's landed cycle-7 doc (run 8f151ba4ac47, landed 65830f1)
> already owns, so this copy rides renamed with the -run-998b49532b44 suffix per the sibling-collision
> convention. Lineage-id map (landed meanings own ids, minted ids renumbered above the all-lineage
> max rm-247): **rm-143 → rm-249** (push-only SW substrate, carried open — decision joint with rm-138) ·
> **rm-144 → rm-248** (release-trigger corpus; LANDED at this merge — the pnpm-workspace.yaml half
> converged with main's rm-154, the corpus module + bidirectional lock test are the surviving delta) ·
> **rm-145 → folded into rm-157** (same disposition landed comment a911e1e1 gave the sibling batch's
> identical v0.114-surfacing item) · **rm-146 → rm-250** (preflight-pin per-repo verification, carried
> open — the rm-120 clone-refresh blocker resolved on main 2026-09-24, clone at v0.115.1) ·
> **rm-147 → converged with main's landed hosted-era comment truthing** (no id: every swept site kept
> main's landed wording — this batch's variants assert 'only base-drift.yaml still runs self-hosted',
> falsified by main's later ubuntu-24.04 pins; the KEEP-self-hosted runner-class decision recorded
> below is MOOTED by that same migration and is preserved as authored history) · **rm-135's dependabot
> half → landed as a rider on main's rm-135** (cooldown.default-days: 1, all three update blocks).
> The dated signals and status flips below were measured at 64024a5/7809df6 and are superseded by
> main's fresher ledger (the 2026-09-24 manual-revision truing) — preserved verbatim as the phase record.
