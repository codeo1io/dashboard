...
> **Landing reconciliation (integrate merge into 8bbff5c6, 2026-09-25).** The batch
> landed behind two parallel lineages: main absorbed the same items via cycles
> 13/cycle:1 (ids rm-181..rm-186 already owned by landed meanings) before this
> branch integrated. This batch's six items are renumbered at the merge —
> rm-181→rm-191 (pins), rm-182→rm-192 (drill-down), rm-183→rm-193 (parsers),
> rm-184→rm-194 (approvals aging), rm-185→rm-195 (sparkline), rm-186→rm-196
> (in-range lockfile refresh); ids in the historical rows below are the batch's
> own at 480c92c and are left verbatim. Field-name convergence: main's landed
> `enumerationIncomplete` MonitoringDto channel (cycle-13 B1, ids-based via
> `failedInstallationIds`) supersedes the batch's parallel `degradedInstallations`
> count — the merged tree keeps the landed field and carries the batch's
> warm-empty/absence semantics under it (see rm-112's FIELD CONVERGENCE note).
