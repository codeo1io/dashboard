<!-- LANDING RENAME 2026-09-25 (integrate of run d6455cdc candidate ed88959,
conflict case 331964b6): this batch doc was authored as
`2026-09-24-cycle-11-batch.md` in the d6455cdc lineage (campaign 22c63cb7
cycle 1, PR #113) while the repo-landed cycle-11 doc (run 41c7d471) already
owns that path on origin/main — add/add at the integrate. The landed doc keeps
the canonical path; this one is renamed with the run id, the
lineage-disambiguation convention (cf. 2026-09-24-cycle-11-batch-run-b4aac9ba.md).
Id reconciliation at this integrate: the batch minted rm-177..rm-183 above a
then-max rm-176 that cycles 11-17 have since consumed with landed meanings, and
every implemented repair had already landed by content (matcher -> rm-168,
shutdown -> rm-171, shell cache -> rm-172, replay un-ack -> rm-169, mint split
-> rm-170; the batch's rm-182 folds into rm-157 and its rm-183 survives as
rm-218 — renumbered off the integrate's original rm-216 mint at independent
review, because the parallel 6cbc2a1d integrate claims rm-216/rm-217 and lands
first). Full dispositions in the INTEGRATE-MERGE comment of ROADMAP.md; the
content below is preserved verbatim, not duplicated into the ledger. -->
