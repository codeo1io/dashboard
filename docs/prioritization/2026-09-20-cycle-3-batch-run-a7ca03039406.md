<!-- LANDING HEADER (2026-09-26, integration 6d785798d31e, conflict case e8b90ceb): this is run a7ca0303's cycle-3 batch artifact, landed under a `-run-a7ca03039406` suffix because a PARALLEL cycle-3 run (f69cd740) authored the same-named doc on the same date and ITS landing owns the canonical path docs/prioritization/2026-09-20-cycle-3-batch.md. The batch sat failed ~6 days (base 7de0de3, 2026-09-20) while main advanced 107 commits through cycles 3-17 plus fourteen sibling integrates, and landed every one of its seven units by content under other ids. NOTHING from this batch's code surface survives into the merged tree — every src/ and test/ conflict resolved to main's landed truth byte-for-byte. Frame->landed map (batch ids rm-125..rm-131 as minted by its B0; ALL collide with landed meanings, the whole mint block is released, zero renumbers since nothing lands as new content):
  - B1 / theirs rm-125 (wiki-writer invariant guard, test/wiki-writer-guard.test.ts) -> SUPERSEDED by main's landed rm-131 test/fork-exclusion-guard.test.ts (2026-09-21, evolved through rm-213's re-injection catch). Their tree-walking scanner is NOT landed: a duplicate implementation of the landed deliberate fixed-scope design (EXCLUSION_FILES allowlist, historical docs mentions out of scope by design), and its header cites their dead frame id 'ROADMAP rm-125' (which on main is the completed privacy-module item). Main's ledger already records the run-93069e23 guard as never-landed and reimplements it — that record stays true.
  - B2 / theirs rm-112 fold + rm-128 (bounded pool, 2x-interval cache TTL, negative cache, cycleMs/skippedCycles telemetry, unresolvedCount, enumeration-failed warm-empty preserve) -> EVERY half superseded or explicitly DECLINED on main: bounded pool landed by content as rm-141 refreshConcurrency; telemetry landed as rm-156 refreshDurationMs/refreshDegraded + rm-203 deadline/stall-watchdog; warm-empty + enumeration-failure landed STRONGER as rm-112's cycle-13 server surface (failedInstallationIds->enumerationIncomplete, resolver-failure absence entries, staleBanner on partial, rm-197 cold-start); the CACHE-TTL raise and NEGATIVE CACHE are declined at rm-112/cycle-9 (per-repo cache REMOVED as dead weight — a raised TTL was rejected; negative caching for dead installations declined — the 60s cadence bounds re-hits). Their src/github/aggregator.ts, app-client.ts, routes/api.ts, server.ts deltas NOT taken; their companion solution doc docs/solutions/best-practices/aggregator-refresh-hardening-pool-ttl-negative-cache-2026-09-20.md NOT landed (it documents exactly the declined/superseded mechanics; the decision record lives at rm-112/rm-141 in ROADMAP and in-tree in src/github/aggregator.ts).
  - B3 / theirs rm-129 (metadata object-entry else-branch counting) -> SUPERSEDED: the identical defect was found and landed on main as rm-130 (2026-09-21, run 2ae7d10a) — metadata.ts carries the trailing else + skippedCount warning, tests cover scalar/array junk AND the structurally-invalid PUBLIC object entry AND private-with-deny-key routing; rm-184's fail-closed private coercion + rm-161's uncovered-entry telemetry are beyond this batch's form.
  - B4 / theirs rm-130 (populate /api/healthz lastFetch/rateLimit + app-client RateLimitState observability) -> DECLINED on main: rm-107's landed decision keeps healthz liveness-only (null placeholders; README truthed 2026-09-25 cycle-15 R1 rider) with the composed operator status panel (rm-107, still candidate) as the recorded home for those signals. The auto-merged healthz-population tests in test/dashboard.test.ts were reverted to main's landed suite (FIFTH convergence of this parallel-shape family — cf. the FOURTH at case e7b03a2f / 449ed1a5). The RateLimitState rider is not landed — it would be dead code pending rm-107's panel; the design survives below in section 4.
  - Rider: .github/renovate.json5 deletion -> no-op (main already deleted it; rm-136 keep-deleted policy + rm-213 guard own the rationale). .github/dependabot.yml comment delta NOT taken (main's rm-133/rm-139 header + ignore rules are the landed form; their comment narrates this batch's own deletion, which did not land from here).
  - Deferred candidates: theirs rm-126 (privacy-policy absorb) landed on main as rm-125 (privacy module + /privacy route, 2026-09-21); theirs rm-127 (PWA offline router absorb) was decided by main's rm-128 — workbox precache dropped, zero workbox-* deps, router declined not absorbed; theirs rm-131 (listener SSE push digest) NOT CARRIED as a ledger item — main landed the polling-hygiene direction (rm-155, 2026-09-26) and rm-107 remains the open monitoring surface; the full idea is preserved in the sibling ideation artifact docs/ideation/2026-09-20-repository-extensions-research-2-run-a7ca03039406.md.
  The B0 registry repair itself is superseded: the status flips it recorded for rm-121/rm-122/rm-124 had already landed on main, and its 2026-09-20 id allocations were superseded by the f69cd740 lineage's landing at f334fb2. Ledger integrity at this merge: no statuses flipped, no ids minted or renumbered, ROADMAP census unchanged (126 unique id definitions, 0 dups, all-lineage max rm-247). The original artifact follows unmodified. -->
