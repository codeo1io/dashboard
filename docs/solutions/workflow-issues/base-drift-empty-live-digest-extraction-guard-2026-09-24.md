...
resolution: |
  Fixed in the cycle:1 batch (rm-182, 2026-09-24) in `.github/workflows/base-drift.yaml`:
...
> Merge note (integrate e21577caef65, 2026-09-24): item id renumbered rm-177 → rm-182 at landing (landed cycle-12 meanings own rm-177..rm-180), and this repair CONVERGED with the same-day rm-166 (cycle-11) + rm-178 (cycle-12) repairs. The landed workflow keeps `docker buildx imagetools inspect` as the PRIMARY readback — its pretty dump's `Digest:` header line is the only form proven on both runner classes, and test/base-drift-digest-readback.test.ts fences `docker manifest inspect` out of the tree entirely. This run's contributions folded in at the merge: the auth.docker.io anonymous-token + registry-1.docker.io HEAD fallback (below) and the `|| true`-inside-substitution discipline; total extraction failure still reds with the distinct NOT-drift message.
