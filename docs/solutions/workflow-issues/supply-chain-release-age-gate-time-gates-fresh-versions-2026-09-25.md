...
During the 2026-09-25 cycle-1 batch (run 4e7c674, rm-186, renumbered rm-196 at the integrate merge), the planned
...
- **Derive publish + maturity timestamps before scheduling a refresh.**
  ...
  exact unlock timestamp in the roadmap item (see rm-196's status line for the
...
- **The right next-cycle rider is fully derivable.** When an item lands
  ...
  (`pnpm update hono vite` plus the manifest pin). rm-196's 2026-09-25 update
...
- rm-196 (renumbered from rm-186 at the integrate merge into 8bbff5c6; the partial-landing item this was learned from; hono 4.13.9 /
