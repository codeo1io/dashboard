...
Landed state (2026-09-23 resolution): the run that authored this lesson
(f4622d7e, rm-100a) stranded in its worktree and main landed the equivalent
fix through run 270220e7 at 916783f — using the OTHER legal form: a job-level
`env: HAS_FRO_BOT_PAT: ${{ secrets.FRO_BOT_PAT != '' }}` with the gating done
by STEP-level `if: env.HAS_FRO_BOT_PAT != 'true'` (the release.yaml 886c28e
contract). That is what fro-bot.yaml on main ships today; prefer it when
per-step skips suffice. Reach for the needs-gate pattern above only when the
WHOLE job must be skipped at job level. Step-level token references (for
example `token: ${{ secrets.FRO_BOT_PAT }}` inside a step) remain legal and
can stay untouched in either form.
