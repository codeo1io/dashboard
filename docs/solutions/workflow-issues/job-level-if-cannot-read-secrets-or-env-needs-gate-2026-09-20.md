...
- The gate job is a real job: it queues on a runner, it appears in
...
Landing history: this pattern shipped 2026-09-20 in the stranded run-f4622d7e
batch (rm-100a; restored record at
`docs/prioritization/2026-09-20-cycle-2-batch-3.md`). What landed on main
(916783f, run 270220e7) instead gates the same secret presence with job-level
`env` + step-level `if: env.HAS_FRO_BOT_PAT` — equally schema-legal, because
job-level `env` MAY read `secrets`; only the job-level `if:` cannot — and it
queues no extra job. fro-bot.yaml on main uses that form; release.yaml's
HAS_RELEASE_APP contract is the same shape. Both escapes satisfy the rule:
prefer the env-at-step-level form when steps can branch individually, the
needs-gate when the whole job must be skipped or ordered behind the check.
Step-level token references (for example `token: ${{ secrets.FRO_BOT_PAT }}`
inside a step) remain legal and were left untouched.
