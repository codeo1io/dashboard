...
jobs:
  secret-gate:
    ...
    runs-on: ubuntu-latest   # historical form said self-hosted; ALL fork CI
                            # moved to hosted runners 2026-09-22 (PRs #11/#12)
    timeout-minutes: 5
    permissions: {}
    outputs:
...
- The gate job is a real job: it queues a runner slot, it appears in
  ...
  (seconds each). (Historically the fork had ONE self-hosted runner, so gate
  jobs serialized behind real work — moot since the 2026-09-22 move to
...
## Which form actually landed

Two schema-valid escapes exist, and BOTH have been used on this fork:

- **needs-gate job** (this doc's pattern) — shipped 2026-09-20 in the stranded
  run-f4622d7e landing branch (rm-100a), which was integrated 2026-09-23 with
  its workflow conflicts resolved to main's form (the batch's substance had
  already landed via run 270220e7 at 916783f).
- **job-level `env` + per-step `if`** — what origin/main actually carries in
  fro-bot.yaml since 916783f (run 270220e7): job-level
  `env: HAS_FRO_BOT_PAT: ${{ secrets.FRO_BOT_PAT != '' }}`
  (legal — `secrets` IS available in job-level `env`) plus
  per-step `if: env.HAS_FRO_BOT_PAT == 'true'` gates and a warn-and-skip step,
  mirroring release.yaml's HAS_RELEASE_APP contract. Prefer this form when a
  whole-job skip is not needed; use the needs-gate when the gate must live in
  the job-level `if:` itself.
...
container form works on any runner, hosted or self-hosted).
