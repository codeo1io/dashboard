...
<!--
LANDING NOTE 2026-09-25 (integrate of run c2ae28ab, conflict case 6a7122e8):
authored in the c2ae28ab cycle-10 batch (PR #97) and validated green there;
landed at the integrate because it is the standalone statement of the
two-digest-class lesson (index vs per-platform manifest). Instance 3 below
('latent: base-drift.yaml's first scheduled fire') was cured on main before
this landing by the rm-166/rm-178/rm-182 readback repairs (fallback `docker
buildx imagetools inspect` + `^Digest:` parse, empty-extraction-is-error
guards, fenced by test/base-drift-digest-readback.test.ts) — read this
instance as history, not current state; the readback recipes in the Solution
section agree with main's landed forms.
-->
