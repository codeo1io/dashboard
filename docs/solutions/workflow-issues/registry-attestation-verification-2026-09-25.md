...
tags: [registry, attestation, provenance, sbom, ghcr, supply-chain, imagetools]
...
never populates (no repo secrets — the App-token publication gate is documented in `docs/solutions/workflow-issues/job-level-if-cannot-read-secrets-or-env-needs-gate-2026-09-20.md`). Registry-side inspection above is the
authoritative check.
