...
tags: [ci, release, guard-drift, corpus-sync-test, rm-248, rm-154, rm-167]
---

> **Provenance (2026-09-26 integrate, conflict case 9f604f39):** authored by run 998b4953's
> cycle-7 batch under its lineage id `rm-144`; main's landed rm-144 (property suites) owns that
> id, so the work lands as **rm-248**. The pnpm-workspace.yaml half recorded below CONVERGED with
> main's rm-154, which added it to both the workflow filter and the guard three days after this
> doc's assess — this doc's surviving content is the single-source corpus module + the
> bidirectional lock test. The lockfile paragraph is superseded on main by rm-167; corrected
> inline at the bottom. Post-landing seam (conflict case 68adf406): main's rm-154 guard test
> `test/release-trigger-paths.test.ts` text-parsed the guard script's INLINE `filePath === '…'`
> literals — a shape the corpus move erased — and went 3/8 red on the merged tree; its parsers
> now read `scripts/release-paths.ts` (line comments stripped, so a commented-out entry is a
> corpus member of nothing) unioned with the legacy inline shape, so both guards hold under
> either layout.
...
2. Define the corpus ONCE and lock it BIDIRECTIONALLY: the hard-path corpus
   ...
   workflow and, since main's rm-167 (cycle-9), the guard FAILS OPEN on it: a
   lockfile-only change releases, and a devDeps-only manifest edit WITH the
   lockfile in the diff also releases — the skip requires an unchanged
   lockfile). Additions AND removals on either side fail CI with a named diff.
