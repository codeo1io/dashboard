...
> **Landed-state note (2026-09-26 cycle-8 integrate).** This batch landed two
> days late; main had independently landed two of the three fixes under other
> ids. #1 landed as written here (`redactedDatabaseIdIn`, rm-151). #2 landed on
> main as rm-184 with a STRICTER discriminator than the `normalizePrivateFlag`
> below — every value that is not strictly `false` or absent classifies private
> (`privateCoercedCount` telemetry), so quoted `'false'` and nonsense values all
> fail closed into the denylist rather than split public/skip; rm-184 is the
> pattern to copy. #3 landed as rm-161 (`redactedEntriesMissingDatabaseId`
