...
cycle-11 batch (rm-172 — the batch's own numbering; that id was reallocated
to the landed SPA-shell-cache item at the 2026-09-25 integrate, the field
itself landing via cycle-13's content rescue of this batch) added
`failedInstallationIds` to `EnumerateReposResult`, production code
(`runRefresh` reading `.failedInstallationIds.length`) threw on the fakes. The implement phase's
