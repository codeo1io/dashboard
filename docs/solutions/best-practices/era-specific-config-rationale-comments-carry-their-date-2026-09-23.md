...
4. **Replaced, not appended.** When fixing, rewrite the stale comment to state the new measurement
   (done in cycle-7 rm-142: the comment now cites the 5m0s/11m15s ubuntu-latest numbers) instead of
   stacking a second comment on top of the corpse of the first.
...
- `ROADMAP.md` rm-142 (cycle-7) — the fix and its post-merge timing-evidence acceptance
