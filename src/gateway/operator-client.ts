 ...
 *
 * connectRunStream is likewise a retained dead surface here: zero non-test
 * callers (the browser clients open their own streams). It was kept when
 * cycle-10 pruned launchRun/getRunSnapshot and is the next-prune candidate;
 * until then the never-log-tool-args invariant below is a documented
 * contract, not a currently-tested one.
 *
 * Security invariants:
