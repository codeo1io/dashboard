...
export function createListenerStore(dbPath: string): ListenerStore {
  ...
  // Two concurrent ingests with the same (source, dedupe_key) could both run
  // the SELECT, find nothing, and both INSERT — the loser then died on
  // idx_messages_source_dedupe with SQLITE_CONSTRAINT_UNIQUE, surfacing as a
  // 500 to the webhook caller. ON CONFLICT against that partial index (the
  // conflict target repeats its WHERE clause) makes duplicate delivery
  // idempotent by construction; rows with a NULL dedupe_key never match the
  // partial index and always insert fresh.
