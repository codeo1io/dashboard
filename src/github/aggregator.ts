...
export interface AggregatorDeps {
  ...
  /**
   * Per-call deadline for every outbound call in a refresh cycle (rm-197).
   * Defaults to AGGREGATOR_FETCH_DEADLINE_MS (15s).
   */
   ...
   * though no newer cycle has run (rm-197 stall watchdog). Defaults to 2×
...
async function fetchRepoStatus(
  ...
  try {
    // rm-197: every outbound call is deadline-bounded — a hung GraphQL call
    // degrades to a stale row for this repo (the catch below) instead of
    // stalling the refresh cycle forever.
    ...
    if (isVulnerabilityAlertsPermissionError(error)) {
      ...
      try {
        // rm-197: the no-alerts retry is deadline-bounded too.
        const raw = await withDeadline(
...
// ---------------------------------------------------------------------------
// rm-197: outbound deadline machinery
// ---------------------------------------------------------------------------

/**
 * Per-call deadline applied to every outbound call made during a refresh
...
/** Refresh interval (ms) — also the unit the stall watchdog counts in (rm-197). */
export const AGGREGATOR_REFRESH_INTERVAL_MS = 60_000

/** A cycle still in flight after 2× the refresh interval serves the stale banner (rm-197). */
 ...
 * endpoint can never stall the refresh loop or hold the in-flight guard
 * forever. True cancellation happens at the client layer where an
 * AbortSignal is available (the real graphql/metadata calls in server.ts).
 */
...
export function createAggregator(
  ...
  // rm-197 stall watchdog: wall-clock start of the currently in-flight
  // cycle (null when idle). getSnapshot() serves the stale banner once a
  // cycle has been running longer than staleAfterMs — previously a cycle
  // wedged on a hung outbound call would hold the in-flight guard forever
  // and the last-good snapshot would keep presenting itself as fresh.
  ...
  async function runRefresh(): Promise<void> {
    // 1. Read metadata + denylist FIRST (rm-197: deadline-bounded)
    ...
    if (metadataResult instanceof DeadlineExceededError || isErr(metadataResult)) {
      ...
      // deadline) — we MUST NOT build a fresh union
    ...
    // 2. Enumerate installation repos (rm-197: deadline-bounded)
    ...
    if (enumerateResult instanceof DeadlineExceededError) {
      // rm-197: enumeration hung past its deadline — treat exactly like an
    ...
    if (deps.resolveInstallationIdForRepo === undefined) {
      ...
      for (const entry of rawWorkingSet) {
        ...
        // Metadata-only repo: resolve installation_id via App JWT (rm-197:
        // resolver calls are deadline-bounded — a hung resolver surfaces an
        // absence entry instead of wedging the cycle).
        try {
    ...
    // row set stays deterministic no matter how the pool interleaves.
    // rm-197: each fetchRepoStatus call is deadline-bounded internally, so
    // every worker slot is guaranteed to free up; Promise.all can never hang.
    ...
    for (const [index, entry] of workingSet.entries()) {
      ...
      dashboardRepos.push({
        ...
        full_name: entry.full_name,
        discovery_channel: entry.discovery_channel,
        // rm-141: fixed-index read-back; the ?? is unreachable (every index
        // is assigned before Promise.all resolves) and exists only to satisfy
        // noUncheckedIndexedAccess.
        status: statuses[index] ?? absenceStatus(),
      })
    }
   ...
   *
   * rm-197 stall watchdog: while a cycle is in flight longer than
   * staleAfterMs (default 2× the refresh interval), the served snapshot
   * carries the stale banner even though lastGood is unchanged — a wedged
   * cycle must never keep presenting itself as fresh data. (With the
   * per-call deadlines above this window is bounded anyway; the watchdog is
   * the belt to that braces.)
   */
  function getSnapshot(): AggregatorSnapshot {
    // rm-197 stall watchdog — applies on BOTH branches: a cold start wedged
    // past the threshold must not present an unbannered "loading" snapshot
    // either.
    const wedgedPastThreshold = cycleStartedAt !== null && now() - cycleStartedAt > staleAfterMs
    if (lastGoodSnapshot === null) {
      return {repos: [], staleBanner: wedgedPastThreshold, driftCount: 0, enumerationIncomplete: null, refreshedAt: null}
    }
    if (wedgedPastThreshold) {
   ...
   *
   * rm-198: the decision to start this loop at all lives one layer up —
   * server.ts gates it on DASHBOARD_MONITORING_REFRESH. The aggregator
   * itself stays default-on so tests and direct consumers keep today's
   * behavior.
   */
