 ...
 * every snapshot write. Absent → in-memory-only behavior (the pre-bridge
...
export interface AggregatorDeps {
  ...
  /**
   * Optional snapshot persistence (rm-198 boot-time bridge — run 9cb75bf5
   * minted it as rm-200; renumbered onto main's landed restart-bridge id at
   * this integrate) — see {@link SnapshotStore}. Absent → in-memory-only
   * snapshots (the pre-bridge contract).
   */
...
export function createAggregator(
  ...
  //
  // rm-198 boot-time bridge: the last persisted snapshot, if any, seeds the
  ...
  /**
   * Every snapshot write goes through here (rm-198): set + best-effort
   ...
   * disk file stops carrying the name too. Restores the pre-bridge implicit
