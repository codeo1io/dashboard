...
export function createAggregator(
  ...
  async function runRefresh(): Promise<void> {
    ...
    // a strict '<' against a 60s refresh interval). Because the cache was only
    // consulted at the top of this same loop, each entry's age at revisit was
    // always >= the interval — the cache never hit in steady state and was pure
    // dead weight. Raising the TTL instead would serve older data to save
    // negligible API budget (1 query/repo/minute for a small fleet), trading
    // away the dashboard's freshness contract. Removed; every cycle fetches
    // every working-set repo fresh.
