 ...
 *
 *   1. stop the aggregator interval (the existing 'close' listener wiring)
 *   2. close the listener SQLite store
 *   3. stop accepting new connections (`server.close`)
 *   4. exit — with a bounded force-exit timer so a hung connection can never
 *      wedge shutdown past the deadline (defaults to 9s so it fires just
 *      before container managers' ~10s SIGKILL; review findings F3/F4).
 *
...
export interface ShutdownDeps {
  ...
  /**
   * Destroy idle keep-alive sockets once the drain begins (rm-171). Node's
  ...
  /**
   * Destroy ALL remaining sockets at the force-exit deadline (rm-171).
...
export function installShutdownHandlers(deps: ShutdownDeps): readonly string[] {
  ...
  function shutdown(signal: string): void {
    ...
    const timer = setTimeoutFn(() => {
      ...
      // rm-171: long-lived streams (proxied operator SSE) never self-complete
    ...
    // rm-171: destroy idle keep-alive sockets BEFORE close() — they hold the
