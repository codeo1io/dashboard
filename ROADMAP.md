...
<!-- 2026-09-24 extension (conductor run 6cbc2a1d, roadmap phase attempt 60538510; items rm-155..rm-162 sourced
     ...
     signals appended to rm-103/rm-108/rm-116/rm-117) -->

### Fleet self-inclusion: monitor the dashboard repository itself
- id: `rm-155` | track: operator-experience | priority: 79.0 | status: candidate (added 2026-09-24, run 6cbc2a1d)
...
- id: `rm-156` | track: operator-experience | priority: 51.0 | status: candidate (added 2026-09-24, run 6cbc2a1d)
- signals: check runs already carry the needed timing fields under the held checks:read permission (started_at/completed_at, re-run attempts) but nothing aggregates them; the category's observability bar is set by Actionsplane (github.com/shpak-e/Actionsplane, verified 2026-09-24) whose core loop is per-workflow success rate, p50/p95 duration, and flakiness across an org; this fork's flake history (badge truncation-boundary and hung-fetch flakes, 2026-09-23) lives only in integrate-ledger notes, nowhere operator-visible; depends on rm-157 for history across restarts
- acceptance:
...
- id: `rm-158` | track: security | priority: 43.0 | status: candidate (added 2026-09-24, run 6cbc2a1d)
...
- id: `rm-159` | track: operator-experience | priority: 42.5 | status: candidate (added 2026-09-24, run 6cbc2a1d)
...
- acceptance:
  ...
  - snapshot carries per-repo cache size/count with a needs-attention trigger at an operator-configured threshold; sustained-growth (thrash) detection deferred to rm-157 history and noted in the batch doc
...
- id: `rm-131` | track: reliability | priority: 88.0 | status: completed (review-F6 reconciliation 2026-09-24: the guard IS landed on origin/main — present at 763c1fa, and its firing is exactly what reddened the four push-Mains b185335→763c1fa on the tracked .conductor breadcrumb, runs 35840489685/35877660739/35878320796/35883003173 — the earlier 'unlanded' status predated the integrate chain that carried it to main; moved to Completed by the 2026-09-24 review-fix pass)
