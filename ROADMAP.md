...
- evidence: dependabot.yml diff plus actionlint container validation (repo convention); gh api on the config returns the ignore list; first weekly PRs after the change propose minor/patch only<!-- cycle-1 extension (2026-09-24, conductor run d6455cdcf51c461b917fcd1250c6db0f roadmap attempt fe7338eb; items rm-177..rm-183 sourced from this run's assess (0047fc4a, 6 fresh findings against origin/main 539c632) + research (d17928f4, 6 evidence-backed candidates); dated signals appended to rm-103/rm-108/rm-117/rm-120/rm-139/rm-140/rm-144/rm-157/rm-165; ids minted above the all-lineage max rm-176 per the open-PR audit — PR #82 carries rm-169..rm-176, PR #104/#97 carry rm-166/rm-167, PR #43 carries rm-160..rm-163, canonical origin/main 539c632 tops out at rm-165; delivered as spool patch + full file, worktree tree untouched per this work order's canonical-hygiene clause) -->

### Fix the vulnerabilityAlerts permission-error matcher (graceful degradation broken for one error string)
- id: `rm-177` | track: security | priority: 76.0 | status: implemented (2026-09-24, run d6455cdc implement dc9bc8c5; validated — focused 141/141, targeted 706/706, full CI 9/9 green via ephemeral PR #107 on base ed86222; pending landing)
- signals: src/github/aggregator.ts:492-504 — isVulnerabilityAlertsPermissionError requires one of 'vulnerabilityalerts' | 'vulnerability_alerts' | 'vulnerability alerts' or ('push access' AND 'vulnerability'); the comment at :497 claims it also covers GitHub's bare "Resource not accessible by integration" — that string matches NONE of the alternatives (case-insensitive), so a missing vulnerabilityAlerts grant surfacing as the bare integration error skips the no-alerts retry at :570-583 and the repo is marked stale EVERY 60s cycle instead of serving alerts null; AGENTS.md invariant 1 promises "security_events/vulnerability_alerts:read optional + graceful"; test/aggregator.test.ts:642-665 pins the degraded behavior — the mock throws exactly the bare message and the test asserts /marking stale/i with no retry
- acceptance: the matcher recognizes the bare integration error (add the literal, keep the match case-insensitive and scoped to the permission-grace path — not a blanket error swallow); a red-then-green test drives the fix — the bare-message case enters the no-alerts retry and yields openAlertCount null with no staleness; the existing stale-path test re-scoped to a genuinely non-permission error; the comment's claim and the matcher agree (grep the grace path for other comment/matcher divergences of this class)
- evidence: test/aggregator.test.ts diff with the new case; the matcher diff; full suite green at the pushed sha

### Wire graceful shutdown (SIGTERM handler; the close-listener stop path is dead code)
- id: `rm-178` | track: reliability | priority: 44.0 | status: implemented (2026-09-24, run d6455cdc implement dc9bc8c5; validated — focused 141/141, targeted 706/706, full CI 9/9 green via ephemeral PR #107 on base ed86222; pending landing)
- signals: src/server.ts:1180 registers server.addListener('close', …) to stop the aggregator, but nothing ever calls server.close() — grep for SIGTERM|SIGINT|process.on across src/ scripts/ returns zero hits — while Dockerfile:88 runs `CMD ["node", "src/server.ts"]` as PID 1, where the kernel applies no default dispositions: an unhandled SIGTERM is dropped, so docker stop always burns the full stop-timeout and then SIGKILLs; the aggregator's stop() path can never execute and an in-flight per-repo refresh is cut hard mid-cycle
- acceptance: a SIGTERM (and SIGINT) handler calls server.close(), closes idle connections (Node 24 closeAllConnections), lets the existing close listener stop the aggregator, and exits within a bounded drain deadline (force-exit past it); a test drives the signal path and asserts the stop chain ran; the deployment note in docs/runbooks/gateway-access.md covers the PID-1 reality (an infra-side init: true mitigates the drop, the in-code handler lands regardless for parity)
- evidence: handler chain test; a local kill -TERM smoke showing clean exit before the stop-timeout; suite green

### Stop reading index.html from disk per request (push-enabled / handler)
- id: `rm-179` | track: performance | priority: 30.0 | status: implemented (2026-09-24, run d6455cdc implement dc9bc8c5; validated — focused 141/141, targeted 706/706, full CI 9/9 green via ephemeral PR #107 on base ed86222; pending landing)
- signals: src/server.ts:828-833 — when pushNotificationsEnabled, the `/` handler runs existsSync + readFileSync(indexHtmlPath, 'utf8') on EVERY request to inject the push script and fix Content-Length; index.html changes only at build/deploy time, so every dashboard load pays two blocking syscalls on the event loop
- acceptance: the injected HTML is cached — memoized at first read or mtime-checked — preserving the Content-Length correctness the comment documents; the chosen invalidation policy named in a comment (startup-only cache is acceptable if the tradeoff is stated); a test asserts byte-identical bodies across repeated requests and, if mtime-checked, a simulated file change invalidates
- evidence: server.ts diff; response-equality test; invalidation-policy note in the diff

### Listener dedupe upsert must not un-ack read messages on replay
- id: `rm-180` | track: reliability | priority: 32.0 | status: implemented (2026-09-24, run d6455cdc implement dc9bc8c5; validated — focused 141/141, targeted 706/706, full CI 9/9 green via ephemeral PR #107 on base ed86222; pending landing)
- signals: src/listener/store.ts:92-94 — the message upsert on dedupe-key match still executes SET … read_at = NULL and refreshes received_at; src/listener/ingest-auth.ts enforces a 300s freshness window but keeps no nonce/seen-signature cache, so a producer retry or replay inside the window re-runs the upsert: a previously-read message flips back to unread and reorders to the inbox top by received_at — for an identical-bytes retry after a network timeout the operator loses ack state with no signal anywhere
- acceptance: the upsert is a no-op for identical content — read_at preserved when the row was read and content unchanged (reset only on a real field change, or never reset read_at at all); received_at preserved on identical replay; tests drive the replay case (same bytes re-POSTed inside the window) and a genuine content-change case, pinning both; the ingest freshness window semantics unchanged
- evidence: store.ts diff with the conditional upsert; the two new tests; suite green

### Separate transient mint failures from missing-scope failures in the token fallback
- id: `rm-181` | track: security | priority: 40.0 | status: implemented (2026-09-24, run d6455cdc implement dc9bc8c5; validated — focused 141/141, targeted 706/706, full CI 9/9 green via ephemeral PR #107 on base ed86222; pending landing)
- signals: src/github/installations.ts:159-195 — mintReadOnlyToken attempts the full read set then falls back to the core set on ANY failure of the first mint; a transient network error or 5xx on the full-scope mint is indistinguishable from a scope rejection, so recovery succeeds with a token silently MISSING security_events/vulnerability_alerts reads for up to the ~55-min cache TTL — alerts vanish (null) instead of the cycle failing loudly; pairs with rm-177: both corrupt the same graceful-degradation contract from opposite ends
- acceptance: scope-class failures (403/404 with the permission error shape) take the core fallback; transient failures (network, 5xx, rate-limit) retry the SAME scope set with bounded backoff before ever degrading; the fallback logs distinguish the two causes; tests pin both paths — a 500-then-success full mint must NOT drop scopes, a scope rejection must still degrade gracefully
- evidence: installations.ts diff; the two path tests; one example log line per cause

### Surface run-status checkout provenance in operator run cards (gated on rm-157)
- id: `rm-182` | track: product | priority: 38.0 | status: candidate (added 2026-09-24, run d6455cdc research C1; SEQUENCED — blocked on rm-157's v0.115.0 pin + contract verification)
- signals: fro-bot/agent v0.115.0 (PR #1656, gh api 2026-09-24) appends gateway run checkout provenance to every agent response and the operator run-status contract now carries it plus a workspace-preparation failure distinction — the dashboard's run cards and stream can answer "which code did this persistent workspace actually run against" (starting commit/branch/detached HEAD, dirty/mid-git-op flag); the fork's RunSnapshotDto is an unvalidated `as T` cast marked MOCK-ONLY/DEFERRED, so consuming new fields is low-risk; v0.114.0's incomplete-invocation outcome (replaces false success for uncertain lifecycle evidence) should also render distinctly from success
- acceptance: after rm-157 lands, run cards + the live stream show checkout provenance (short sha, branch/detached, dirty badge); workspace-prep failures and incomplete-invocation outcomes render as distinct states, never green; provenance fields are absent-tolerant (older gateway pins degrade silently); web tests cover the present/absent render matrix
- evidence: operator-client/contract re-check against the v0.115.0 clonedep refresh (rm-120); web component tests for the render matrix; a live-session or runbook screenshot note

### Fleet rulesets-coverage panel (read-only, zero permission additions)
- id: `rm-183` | track: product | priority: 46.0 | status: candidate (added 2026-09-24, run d6455cdc research C4)
- signals: REST GET /repos/{owner}/{repo}/rulesets works with GitHub App installation tokens requiring ONLY Metadata(read) — the fine-grained token table on docs.github.com/en/rest/repos/rules fetched 2026-09-24 — and the minted read-only subset already carries metadata:read, so a fleet-wide "which repos have enforced rules on their default branch" panel needs zero permission additions and cannot write; classic branch-protection REST would need administration(read) — excluded by design; cross-ref rm-116 (the dashboard's own unprotected main caused three red landings): the panel generalizes that lesson from one repo to the whole working set
...
- evidence: aggregator diff with the rulesets fetch; attention-view rendering; test matrix; a docs note that classic branch-protection is intentionally out of scope (administration read never granted)
