...
- signals: IMPLEMENTED 2026-09-24 (run b4aac9ba implement 416c2758, batch B1): override landed in `pnpm-workspace.yaml` `overrides:` — pnpm 11 silently ignores `pnpm.overrides` in package.json (WARN-only; solution doc docs/solutions/build-errors/pnpm11-overrides-live-in-pnpm-workspace-yaml-2026-09-24.md) — toml re-resolved 4.1.2→5.0.0 sole resolution, `pnpm audit -r` 0 findings; validated on cloud ephemeral PR #70 (all 9 checks SUCCESS, full suite 3126/3126, run 35962834943); NOT yet landed — flips to Completed only when the landing commit reaches origin/main green
...
- signals: IMPLEMENTED 2026-09-24 (run b4aac9ba implement 416c2758, batch B2 anchor): `EnumerateReposResult.failedInstallationIds` (ids only, no account names) populated at both per-install catch sites; `AggregatorSnapshot.enumerationIncomplete: number|null` (count on partial, null on total failure/pre-refresh); new `markSnapshotStale` consolidates metadata-fail-closed + enumeration-failure + unexpected-refresh-throw so a degraded snapshot is never served fresh; MonitoringDto wired additively through src/routes/api.ts + src/server.ts (SPA rendering deferred to the rm-126 consumer decision); 11 new tests, targeted closure 702/702, cloud full 3126/3126 at PR #70; NOT yet landed — stays candidate until the landing commit is green on main
...
- signals: IMPLEMENTED 2026-09-24 (run b4aac9ba implement 416c2758, batch B3): `.gitignore` gains `.conductor/` + `.pnpm-store/`, fork-exclusion-guard extended to assert both entries survive upstream absorbs; validated in the cloud full run at PR #70 (guard green in the ephemeral snapshot because .conductor/** is excluded from validation snapshots); LANDING RIDER still required and unaffected by this prevention: origin/main 82312f7 already TRACKS 13 ndjson — the landing merge must carry `git rm -r --cached .conductor` or Main stays RED (35954752299); ignore entries only prevent future adds, they cannot untrack
