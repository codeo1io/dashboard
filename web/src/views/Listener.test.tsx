...
describe('ListenerChannel', () => {
  ...
  it('rm-198: shows the contract-drift notice when the client drops a malformed message', async () => {
    vi.mocked(listenerApi.fetchListenerMessages).mockResolvedValueOnce({
      ok: true,
      data: {
        messages: [
  ...
  it('rm-199: shows the retention notice when the server reports pruned rows', async () => {
    vi.mocked(listenerApi.fetchListenerMessages).mockResolvedValueOnce({
      ok: true,
